                        ...
                        <div class="d-flex algin-center space-between">
                            <div class="order__detail--action">
                                @if ($order['order_status'] == 0)
                                    <a href="{{ route('get_admin.order.sendGHN', $order['order_id']) }}" class="btn btn-outline-dark btn-fw">Gửi đơn vị giao hàng</a>
                                @elseif ($order['order_status'] <= 2)
                                    <a href="{{ route('get_admin.order.update', $order['order_id']) }}" class="btn btn-outline-dark btn-fw">Duyệt đơn</a>
                                @else
                                    <a href="{{ route('get_admin.order.detroy', $order['order_id']) }}" class="btn btn-outline-dark btn-fw">Trả hàng</a>
                                @endif
                                <a href="{{ route('get_admin.order.print', $order['order_id']) }}" target="_blank" class="btn btn-outline-dark btn-fw mg-l-16">In Hóa Đơn</a>
                            </div>

                            <div class="order_status">Tình trạng đơn: <span class="col-span">{{ format_order_status($order['order_status']) }}</span></div>
                        </div>
                    </div>
                </div>
            </div>
